...
def test_disk_target_capacity_uses_conservative_published_formula_table():
    result = size_nas(
        NasInput(
            source_tb=50.0,
            share_count=20,
    ...
    # 50 TB full + (50 * 5% * 14) = 85 TB backup data.
    # Conservative BP formula table: +10% metadata +10% workspace = 102 TB.
    assert result.primary_repo_tb == 102.0
    assert result.cache_repo_tb == 0.0
    assert result.total_repo_tb == 102.0
    assert any("documented-source ambiguity" in note for note in result.notes)


def test_object_target_capacity_and_active_metadata_cache_known_answer():
    result = size_nas(
        NasInput(
            source_tb=50.0,
            share_count=20,
    ...
    # 85 TB backup data + 5% object metadata = 89.25 TB.
    assert result.primary_repo_tb == 89.2
    assert result.cache_repo_tb == 0.006
    assert result.total_repo_tb == 89.3
...
def test_object_target_changes_cache_repository_compute_not_proxy_formula():
    disk = size_nas(
        NasInput(source_tb=50.0, share_count=20, concurrent_sources=1, object_storage=False)
    )
    obj = size_nas(
        NasInput(source_tb=50.0, share_count=20, concurrent_sources=1, object_storage=True)
    )
...
def test_proxy_and_cache_resources_scale_with_explicit_concurrent_sources():
    disk = size_nas(
        NasInput(source_tb=50.0, share_count=20, concurrent_sources=4, object_storage=False)
    )
    obj = size_nas(
        NasInput(source_tb=50.0, share_count=20, concurrent_sources=4, object_storage=True)
    )
