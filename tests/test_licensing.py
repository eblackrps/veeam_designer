...
def test_instance_model_counts_generic_machine_workloads():
...
def test_socket_model_requires_supplied_occupied_socket_count():
    unknown = estimate_license(
        LicenseInput(vm_count=600, license_type="socket", occupied_sockets=0)
    )
    supplied = estimate_license(
        LicenseInput(vm_count=600, license_type="socket", occupied_sockets=12)
    )

    assert unknown.estimated_sockets == 0
    assert supplied.estimated_sockets == 12
    assert all("VM count cannot be converted" not in note for note in supplied.notes)
...
def test_license_cost_is_never_invented():
    ...
    assert result.annual_maintenance_usd == 0.0
    assert any("Commercial pricing is intentionally not estimated" in note for note in result.notes)
