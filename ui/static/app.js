...
function updateEditorModeNote() {
  ...
  note.textContent =
    getEditorMode() === "builder"
      ? "Form fields are synchronized to YAML."
      : "Manual YAML is enabled. Select Refresh YAML to replace it with the current form values.";
}
...
function collectVmSites() {
  return getSiteCards().map((card, index) => ({
    ...
    years_to_plan_for: getCardNumber(card, "years_to_plan_for", 1),
...
function buildVmSiteYaml(
  ...
  const lines = [
    ...
    `      years_to_plan_for: ${site.years_to_plan_for}`,
...
function renderHumanOutput(target, value) {
  ...
  if (!lines.length) {
    target.innerHTML = '<div class="output-placeholder">No results.</div>';
    return;
  }
  ...
  dashboard.sites.forEach((site) => {
    ...
    article.innerHTML = `
      ...
      <dl class="metric-list">
        ...
        ${renderMetric("Modeled Storage Cost / Year", formatCurrency(site.total_yearly_usd))}
      </dl>
    `;
...
function setBusy(isBusy) {
  ...
  runButton.textContent = isBusy ? "Calculating..." : "Calculate";
}
...
function parseProjectInput(text) {
  ...
  if (window.jsyaml?.load) {
    ...
    if (!parsed || Array.isArray(parsed) || typeof parsed !== "object") {
      throw new Error("Input must be a top-level YAML or JSON object.");
    }
    return parsed;
  }
  ...
  const parsed = JSON.parse(raw);
  if (!parsed || Array.isArray(parsed) || typeof parsed !== "object") {
    throw new Error("Input must be a top-level JSON object.");
  }
  return parsed;
}
...
<body>
  ...
  <p>Generated by Veeam Designer · Browser edition.</p>
