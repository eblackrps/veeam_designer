...
def estimate_license(lin: LicenseInput) -> LicenseEstimate:
    ...
    if total < 10:
        ...
        notes.append(f"Standard tier: {total} protected workloads at ~${rate:.0f}/instance/yr list price.")
    else:
        tier = "enterprise"
        rate = float(CONFIG.get("vul_enterprise_price_usd", 120.0))
        notes.append(f"Enterprise tier: {total} workloads — volume pricing applies (~${rate:.0f}/instance/yr).")

    annual_usd = round(total * rate, 2)
    estimated_sockets = max(1, ceil(lin.vm_count / 10))

    if lin.nas_tb > 0:
        notes.append(f"NAS: {lin.nas_tb:.0f} TB counted as {nas_equiv} VUL instances (1 TB = 1 instance).")
    ...
    if lin.license_type == "socket":
        notes.append(f"Socket estimate: ~{estimated_sockets} sockets ({lin.vm_count} VMs / 10 per socket).")
